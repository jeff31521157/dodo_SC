...
contract MysteryBoxV1 is ERC721URIStorage, InitializableOwnable {
    ...
    event DisableRedeem();
    event EnableRedeem();
    event test(uint256 number, uint256 current_ticket);
    event test_random(uint256 random_middle, uint256 random_result, uint256 gas_result);
    ...
    function buyTicket() payable external {
        ...
        emit test(tickets, _CUR_SELLING_TICKETS_);
    ...
    function _redeemSinglePrize(address to) internal {
        ...
        uint256 random_middle = random(gasleft());
        uint256 random = random_middle % range;
        emit test_random(random_middle, random, gasleft());
        //uint256 random = 0;
    ...
    function random(uint256 seed) internal view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(blockhash(block.number-1), msg.sender, seed)));
    }
